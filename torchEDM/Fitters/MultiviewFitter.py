...
class MultiviewFitter(EDMFitter):
	...
	def __init__(self, D: int = 0, EmbedDimensions: int = 1, PredictionHorizon: int = 1, KNN: int = 0, Step: int = -1,
	...
	def Fit(self, X_train, Y_train, X_test = None, Y_test = None):
		self.Result = MultiviewPredict(X_train, Y_train, X_test, Y_test,
									   D = self.D, embedDimensions = self.EmbedDimensions,
									   predictionHorizon = self.PredictionHorizon, knn = self.KNN, step = self.Step,
									   multiview = self.NumMultiview, exclusionRadius = self.ExclusionRadius,
									   isRankedInSample = self.IsRankedInSample,
