...
| `03_ccm_gate.py` | CCM convergence gate per candidate (E, slopes, decisions) | ~25 min* |
...
*on 4 CPU cores; ~2 min when `ref_ccm_all80.pkl` already exists. Each script
prints its expected outcome in the docstring. Run 03 before 04 (04 reuses
`ref_ccm_all80.pkl`).
